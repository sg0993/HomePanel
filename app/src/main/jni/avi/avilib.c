...
#define AVIF_COPYRIGHTED        0x00020000
	...
	OUTLONG(10000000);
	/* MaxBytesPerSec, I hope this will never be used */
	OUTLONG(0);
	...
	if (AVI->a_chans && AVI->audio_bytes) {
		...
		OUTLONG(sampsize);
		/* Scale */
		OUTLONG(sampsize*AVI->a_rate);
		/* Rate: Rate/Scale == samples/second */
		OUTLONG(0);
		/* Start */
		OUTLONG(AVI->audio_bytes/sampsize);
		/* Length */
		OUTLONG(0);
		/* SuggestedBufferSize */
		OUTLONG(-1);
...
avi_t *AVI_open_input_file(char *filename, int getIndex) {
	...
	while (1) {
		...
		if (strncasecmp(data, "LIST", 4) == 0) {
			...
			if (strncasecmp(data, "hdrl", 4) == 0) {
				...
				if (read(AVI->fdes, hdrl_data, n) != n)
				ERR_EXIT(AVI_ERR_READ)
	...
	for (i = 0; i < hdrl_len;) {
		...
		if (strncasecmp(hdrl_data + i, "strh", 4) == 0) {
			...
			if (strncasecmp(hdrl_data + i, "vids", 4) == 0 && !vids_strh_seen) {
				...
				vids_strh_seen = 1;
				lasttag = 1; /* vids */
			} else if (strncasecmp(hdrl_data + i, "auds", 4) == 0
					   && !auds_strh_seen) {
				AVI->audio_bytes = str2ulong(hdrl_data + i + 32)
								   * avi_sampsize(AVI);
	...
	ERR_EXIT(AVI_ERR_NO_VIDS)
	AVI->video_index = (video_index_entry *) malloc(
			nvi * sizeof(video_index_entry));
	if (AVI->video_index == 0)
	ERR_EXIT(AVI_ERR_NO_MEM)
	if (AVI->audio_chunks) {
		AVI->audio_index = (audio_index_entry *) malloc(
...
void AVI_print_error(char *str) {
	...
	aerrno =
			(AVI_errno >= 0 && AVI_errno < num_avi_errors) ?
			AVI_errno : num_avi_errors - 1;
...
/*
 * Class:     com_android_concox_AVIUtil
 * Method:    init
 * Signature: (Ljava/lang/String;)I
 */

JNIEXPORT jint JNICALL Java_com_honeywell_homepanel_ui_AudioVideoUtil_AVIUtil_init(JNIEnv * env,
													 jclass obj, jstring path, jint width, jint height, jint fps) {
	const char* filename = (*env)->GetStringUTFChars(env, path, NULL);
	LOGD("filename:", filename);
	avi_handle = AVI_open_output_file((char*) filename);
	LOGD("%d\n", __LINE__);
	if (!avi_handle) {
		LOGD("%d\n", __LINE__);
		return -1;
	}
	//AVI_set_video(avi_handle, 854, 480, 15, "H264");
	AVI_set_video(avi_handle, width, height, fps, "H264");
	LOGD("%d\n", __LINE__);
	//AVI_set_audio(avi_handle, 1, 8000, 16, WAVE_FORMAT_ALAW);
	AVI_set_audio(avi_handle, 1, 8000, 16, WAVE_FORMAT_PCM);
	LOGD("%d\n", __LINE__);
...
JNIEXPORT jint JNICALL Java_com_honeywell_homepanel_ui_AudioVideoUtil_AVIUtil_writeVideo(JNIEnv * env,
														   jclass obj, jbyteArray video, jint len, jlong videoTime) {
	jbyte * videoBuf = (jbyte*) (*env)->GetByteArrayElements(env, video, 0);
	static int frame_num = 0;
	static unsigned int prev_video_time = 0;
	AVI_set_video_position(avi_handle, frame_num++);
	...
	if (prev_video_time > videoTime + 1000) {
		videoTime += 4294967;
	}
	if (prev_video_time == 0) {
		prev_video_time = videoTime;
	} else {
		while ((prev_video_time + 66) < videoTime) {
	...
	int ret = -1;
	ret = AVI_write_frame(avi_handle, (char *) videoBuf, (long) len);
	if (ret < 0) {
	...
	int ret = -1;
	if(videoTime != 0){
		int count = (videoTime - prev_video_time)/40;
 ...
 * Signature: ([BI)I
 */
JNIEXPORT jint JNICALL Java_com_honeywell_homepanel_ui_AudioVideoUtil_AVIUtil_writeAudio(JNIEnv * env,
														   jclass obj, jbyteArray audio, jint len) {
	jbyte * audioBuf = (jbyte*) (*env)->GetByteArrayElements(env, audio, 0);
	static int frame_num = 0;
	AVI_set_audio_position(avi_handle, frame_num++);
	LOGD("%d\n", __LINE__);
	if (AVI_write_audio(avi_handle, (char*) audioBuf, (long) len) != 0) {
		LOGD("%d\n", __LINE__);
		(*env)->ReleaseByteArrayElements(env, audio, audioBuf, 0);
		return -1;
	}
	LOGD("%d\n", __LINE__);
	(*env)->ReleaseByteArrayElements(env, audio, audioBuf, 0);
...
JNIEXPORT jint JNICALL Java_com_honeywell_homepanel_ui_AudioVideoUtil_AVIUtil_close(JNIEnv * env,jclass obj) {
