...
public class VideoEncoderFromBuffer {
    ...
    private int writeFrameCnt = 0;//for debug, 只写开头部分数据，防止测试把flash写坏。

    @SuppressLint("NewApi")
    public VideoEncoderFromBuffer(int width, int height) throws IOException {
    ...
    public void encodeFrame(byte[] input/* , byte[] output */, IAvRtpService iAvRtpService) {
...
//					}
                    if(writeFrameCnt < 200) { // for debug, protect flash
                        mMuxer.writeSampleData(mTrackIndex, outputBuffer, mBufferInfo);

                        //TODO write to liping artp service
                        try {
                            if(null != iAvRtpService){
                                iAvRtpService.setVideoFrame(outputBuffer.array());
                            }
                            else{
                                Log.e(TAG,TAG +" iAvRtpService is null 111111111! ");
                            }
